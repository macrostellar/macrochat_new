    ...
    try {
      if (Platform.OS === 'web') {
        if (navigator.clipboard && window.isSecureContext) {
          ...
          textarea.value = profile.macroId;
  ...
  return (
    <Screen>
      ...
      <View style={styles.identityCard}>
        ...
        <Text style={styles.identityText}>Your ID: {profile?.macroId}</Text>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Copy your Macro ID"
          hitSlop={16}
          onPress={copyToClipboard}
          style={styles.copyButton}
        >
          <Ionicons name={copied ? 'checkmark' : 'copy-outline'} color={copied ? colors.neon : colors.blue} size={17} />
        </Pressable>
      </View>
      <View style={styles.securityRow}>
        <Pressable style={[styles.securityChip, styles.securityChipOn]} onPress={() => router.push('/security/mfa')}>
          <Ionicons name="shield-checkmark" size={12} color={colors.neon} />
          <Text style={[styles.securityChipText, styles.securityChipTextOn]}>Security</Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  copyButton: { width: 36, height: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, padding: 8 },
  ...
  search: { marginHorizontal: 20, marginTop: 6, marginBottom: 12, height: 48, borderRadius: 15, backgroundColor: colors.navy800, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, gap: 9 }, searchInput: { color: colors.white, flex: 1, fontSize: 15, outlineStyle: 'none', outlineWidth: 0, borderWidth: 0, boxShadow: 'none' } as any,
